...
gst_nvinfer_class_init (GstNvInferClass * klass)
{
  ...
  g_object_class_install_property (gobject_class, PROP_ALIGNMENT,
      g_param_spec_int ("alignment", "Face Alignment Mode",
          "Face alignment mode: -1=disabled, 1=align from full frame, 2=align from object.\n"
          "\t\t\tSet to -1 to disable face alignment.",
          -1, 2, DEFAULT_ALIGNMENT,
          (GParamFlags) (G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS |
              GST_PARAM_MUTABLE_READY)));

  g_object_class_install_property (gobject_class, PROP_USER_META,
      g_param_spec_int ("user-meta", "User Meta Flag",
          "User meta flag for tensor output.\n"
          "\t\t\tSet to -1 to disable.",
...
/* Helper function to calculate scale from SCRFD detection network */
static float det_scale_scrfd(int in_h, int in_w, int out_h, int out_w){
  float im_ratio = float(in_h) / float(in_w);
  float model_ratio = float(out_h) / float(out_w);
  ...
  return float(new_height) / in_h;
}

/* Align face from cv::Mat using landmarks and similarTransform */
static cv::Mat align_object_from_mat(cv::Mat in_mat, float *mask_data, float scale, int size_x, int size_y){
    ...
    float landmark[5][2] = {
      { mask_data[1]/scale, mask_data[0]/scale},
      { mask_data[3]/scale, mask_data[2]/scale},
      { mask_data[5]/scale, mask_data[4]/scale},
      { mask_data[7]/scale, mask_data[6]/scale},
      { mask_data[9]/scale, mask_data[8]/scale}
    };

    cv::Mat src(5, 2, CV_32FC1, norm_face);
    aligned = cv::Mat(size_y, size_x, CV_8UC4);

    cv::Mat dst(5, 2, CV_32FC1, landmark);
    cv::Mat m = similarTransform(dst, src);
    cv::Mat transfer = m(cv::Rect(0, 0, 3, 2));
    cv::warpAffine(in_mat, aligned, transfer, cv::Size(size_y, size_x), 1, 0, 0);

    return aligned;
}

/* Convert aligned faces and push to input thread */
static gboolean
convert_batch_and_push_to_input_thread_object_aligned (GstNvInfer *nvinfer,
    GstNvInferBatch *batch, GstNvInferMemory *mem, std::vector<cv::Mat> aligneds)
{
  NvBufSurfTransform_Error err = NvBufSurfTransformError_Success;
  std::string nvtx_str;
  cudaError_t cudaReturn;
  ...
  eventAttrib.color = NVTX_DEEPBLUE_COLOR;
  eventAttrib.messageType = NVTX_MESSAGE_TYPE_ASCII;
  nvtx_str = "convert_buf_aligned batch_num=" + std::to_string(nvinfer->current_batch_num);
  eventAttrib.message.ascii = nvtx_str.c_str();
  ...
  NvBufSurface * surface = mem->surf;

  for (uint frameIndex = 0; frameIndex < surface->numFilled; frameIndex++) {
      /* Copy aligned face to surface data */
      void *src_data = NULL;
      ...
      if (PLATFORM == 1) /* Jetson */
      {
          NvBufSurfaceMap(surface, -1, -1, NVBUF_MAP_READ);
          memcpy(src_data, (char *)surface->surfaceList[frameIndex].mappedAddr.addr[0], surface->surfaceList[frameIndex].dataSize);
          NvBufSurfaceUnMap(surface, -1, -1);
          cv::Mat frame_surface = cv::Mat(surface->surfaceList[frameIndex].height,
                                            surface->surfaceList[frameIndex].width,
                                            CV_8UC4, src_data,
                                            surface->surfaceList[frameIndex].pitch);
          aligneds.at(frameIndex).copyTo(frame_surface);
          NvBufSurfaceMap(surface, -1, -1, NVBUF_MAP_WRITE);
          memcpy((char *)surface->surfaceList[frameIndex].mappedAddr.addr[0], frame_surface.ptr(0), surface->surfaceList[frameIndex].dataSize);
          NvBufSurfaceUnMap(surface, -1, -1);

      } else if (PLATFORM == 0) /* dGPU */
      {
          ...
          cv::Mat frame_surface = cv::Mat(surface->surfaceList[frameIndex].height,
                                          ...
                                          CV_8UC3, src_data,
                                          surface->surfaceList[frameIndex].pitch);
          cv::cvtColor(aligneds.at(frameIndex), frame_surface, cv::COLOR_RGBA2RGB);
...
/* Process face alignment on objects detected by upstream detectors. */
static GstFlowReturn
gst_nvinfer_align_objects(GstNvInfer * nvinfer, GstBuffer * inbuf,
    NvBufSurface * in_surf)
{
  std::unique_ptr<GstNvInferBatch> batch (nullptr);
  GstBuffer *conv_gst_buf = nullptr;
  ...
  std::vector<cv::Mat> aligneds;
  for (NvDsMetaList * l_frame = batch_meta->frame_meta_list; l_frame != NULL; l_frame = l_frame->next)
  {
    bool mat_converted = false;
    cv::Mat in_mat;
    index_mat ++;
    ...
    for (NvDsMetaList * l_obj = frame_meta->obj_meta_list; l_obj != NULL; l_obj = l_obj->next) {
      ...
      if (not mat_converted){
          /* Convert NvBufSurface to cv::Mat for face alignment */
          NvBufSurfaceParams *surfParams = &in_surf->surfaceList[frame_meta->batch_id];

          if (PLATFORM == 0) { /* dGPU - use cudaMemcpy */
              /* Allocate host buffer */
              size_t dataSize = surfParams->planeParams.pitch[0] * surfParams->planeParams.height[0];
              void *hostPtr = malloc(dataSize);
              if (hostPtr == NULL) {
                std::cout << "Failed to allocate host buffer for face alignment\n";
                continue;
              }

              /* Copy from GPU to CPU */
              cudaMemcpy(hostPtr, surfParams->dataPtr, dataSize, cudaMemcpyDeviceToHost);

              in_mat = cv::Mat(surfParams->planeParams.height[0],
                               surfParams->planeParams.width[0], CV_8UC4,
                               hostPtr, surfParams->planeParams.pitch[0]).clone();
              free(hostPtr);

          } else { /* Jetson - use NvBufSurfaceMap */
              if (surfParams->mappedAddr.addr[0] == NULL){
                if (NvBufSurfaceMap(in_surf, frame_meta->batch_id, 0, NVBUF_MAP_READ) != 0)
                {
                  std::cout << "Failed to map the surface buffer\n";
                  continue;
                }
              }
              if (in_surf->memType == NVBUF_MEM_SURFACE_ARRAY || in_surf->memType == NVBUF_MEM_HANDLE)
              {
              ...
              in_mat = cv::Mat (surfParams->planeParams.height[0],
                surfParams->planeParams.width[0], CV_8UC4,
                surfParams->mappedAddr.addr[0],
                surfParams->planeParams.pitch[0]);
          }

          mat_converted = true;
      }

      /* Face alignment using landmarks from mask_params */
      float *mask_data=(float *)object_meta->mask_params.data;

      int network_width = object_meta->mask_params.width;   //networkInfo width
      int network_height = object_meta->mask_params.height;
      cv::Mat aligned;

      /* Debug: check if mask_data is valid */
      if (mask_data == NULL) {
        std::cout << "DEBUG: mask_data is NULL for object " << object_meta->object_id << std::endl;
        continue;
      }
      // std::cout << "DEBUG: mask_data landmarks: " << mask_data[0] << "," << mask_data[1] << "," << mask_data[2] << "," << mask_data[3] << " network=" << network_width << "x" << network_height << std::endl;

      int width_dest = (memory->surf->surfaceList + idx)->width;
      int height_dest = (memory->surf->surfaceList + idx)->height;

      if(nvinfer->alignment == 1) /* Align from full frame */
      {
        float scale = det_scale_scrfd(in_mat.rows, in_mat.cols, network_height, network_width);
        aligned = align_object_from_mat(in_mat, mask_data, scale, width_dest, height_dest);
        
      } else if(nvinfer->alignment == 2) { /* Align from object crop */
        int left = int(object_meta->parent->rect_params.left);
        int top = int(object_meta->parent->rect_params.top);
        int width = int(object_meta->parent->rect_params.width);
        int height = int(object_meta->parent->rect_params.height);
        cv::Mat object = in_mat(cv::Range(top, top + height), cv::Range(left, left + width));
        float scale = det_scale_scrfd(height, width, network_height, network_width);
        aligned = align_object_from_mat(object, mask_data, scale, width_dest, height_dest);
      } else
      {
        std::cout << "==================== Not support align mode ========================\n";
      }

      /* Debug: save aligned face to file */
      // static int align_count = 0;
      // if (!aligned.empty() && align_count < 5) {
      //     std::string filename = "/home/mq/disk2T/quangnv/face/aligned_" + std::to_string(align_count) + ".jpg";
      //     cv::imwrite(filename, aligned);
      //     std::cout << "DEBUG: Saved aligned face to " << filename << " size=" << aligned.cols << "x" << aligned.rows << std::endl;
      //     align_count++;
      // }
      ...
      if (batch->frames.size () == nvinfer->max_batch_size) {
          if (!convert_batch_and_push_to_input_thread_object_aligned (nvinfer, batch.get(), memory, aligneds)) {
            return GST_FLOW_ERROR;
          }
          /* Batch submitted. Set batch to nullptr so that a new GstNvInferBatch
          * structure can be allocated if required. */
    ...
    NvBufSurface * in_surf)
{
  ...
  for (NvDsMetaList * l_user = batch_meta->batch_user_meta_list; l_user != NULL;
    ...
    nvinfer->maintain_aspect_ratio = preproc_meta->tensor_meta->maintain_aspect_ratio;
    ...
    gboolean discont, GstBuffer * inbuf)
{
  ...
  } else if (nvinfer->alignment > 0) {
    /* Use face alignment processing */
    flow_ret = gst_nvinfer_align_objects (nvinfer, inbuf, in_surf);
  } else {
...
GST_PLUGIN_DEFINE (GST_VERSION_MAJOR, GST_VERSION_MINOR, nvdsgst_infer,
    DESCRIPTION, nvinfer_plugin_init, "7.1", LICENSE, BINARY_PACKAGE, URL)
